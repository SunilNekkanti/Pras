...
now() created_date, now() updated_date,'sarath' created_by ,'sarath' updated_by,'Y' active_ind, :fileId file_id,
 ...
 left OUTER join membership_provider mcprvdr  on  mi.mbr_id=mcprvdr.mbr_id  and mcprvdr.prvdr_id=mc.prvdr_id          
 LEFT outer join hedis_cpt_measure hcptm on hcptm.hedis_msr_rule_Id =  hmr.hedis_msr_rule_Id   
  LEFT OUTER JOIN cpt_measure  cpt on    hcptm.cpt_id =cpt.cpt_id  
  ...
  LEFT outer JOIN icd_measure  icd  on  icd.icd_id   = hicdm.icd_id  
  LEFT outer join  icd_measure MbrClaimICD on and mc.Diagnoses LIKE CONCAT('%', icd.code,'%')   and icd.code !='0'
  ...
  where 
        ...
        case when  hmr.cpt_or_icd = 0 then   hcptm.cpt_id  is not null  and  mbrClaimCPT.cpt_id is null 
             when  hmr.cpt_or_icd = 1 then  hicdm.icd_id is not null and   MbrClaimICD.icd_id is null
             else 1=1 end    and
...
now() created_date, now() updated_date,'sarath' created_by ,'sarath' updated_by,'Y' active_ind, :fileId file_id,
 ...
 hmr.cpt_or_icd,
 group_concat( distinct  mc.diagnoses) diagnosis,
 icd.code ruleicd,
  mp.pbm_id 
 ...
 LEFT outer join hedis_cpt_measure hcptm on hcptm.hedis_msr_rule_Id =  hmr.hedis_msr_rule_Id  and  hcptm.cpt_id = mcd.cpt_code  
  ...
  LEFT outer join hedis_icd_measure hicdm on   hicdm.hedis_msr_rule_Id =  hmr.hedis_msr_rule_Id 
  LEFT outer JOIN icd_measure  icd  on  icd.icd_id   = hicdm.icd_id  and mc.Diagnoses LIKE CONCAT('%', icd.code,'%')   and icd.code !='0'
  LEFT outer Join membership_problems mp on mp.pbm_id =hmr.problem_id and mp.mbr_id=mi.mbr_id  and  mcd.claim_start_date >= mp.start_date
  where  case when  hmr.cpt_or_icd = 2 then    mp.pbm_id is not null and  icd.code is null  else 1=1 end
  and case when  hmr.cpt_or_icd = 0 then  hmr.problem_id is  null and hcptm.cpt_id is not null  else 1=1 end 
  and hmr.active_ind='Y'
  group by mi.mbr_id, mi.ins_id,hmr.hedis_msr_rule_id 
