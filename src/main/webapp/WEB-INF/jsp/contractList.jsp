...
<div class="panel-group">
	<div class="panel panel-success">
		<div class="panel-heading">
			${contractType} <span class="clrRed">${Message}</span>
			<button class="btn btn-danger pull-right btn-xs"
				onclick="return newContract(${pmpmRequired});">
					...
					<tbody id="content">
						...
						<c:forEach items="${contractList}" var="cntct">
							<tr>
										...
										<c:when test="${cntct.activeInd == 89}">
											<c:choose>
												...
												<c:when test="${cntct.referenceContract.prvdr != null}">
													<a
														onclick="return contract(${cntct.referenceContract.prvdr.id},${cntct.id},${pmpmRequired})"
														href="javascript:void(0)" rel='tab'>Edit</a>
												</c:when>
												<c:when test="${cntct.referenceContract.ins != null}">
													<a
														onclick="return contract(${cntct.referenceContract.ins.id},${cntct.id},${pmpmRequired})"
														href="#" rel='tab'>Edit</a>
												</c:when>
												<c:otherwise>
											issue
								...
								<td><fmt:formatDate value="${cntct.startDate}"
										pattern="MM-dd-yyyy" /></td>
								<td><fmt:formatDate value="${cntct.endDate}"
										pattern="MM-dd-yyyy" /></td>
								<c:choose>
									<c:when test="${not empty cntct.filesUpload.id}">
										<td><a href="#" onclick="fileDownload(${cntct.id})"><span
...
<div id="myModal" class="modal fade" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			...
			<div class="modal-body" id="modal-body">${fileContent}</div>
