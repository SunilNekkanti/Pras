...
$(document).ready(function() {
		...
		$("#hospitalizationGenerate").click(function(event)
		{
		 ...
		 var providerDropdown = function(){
    	  ...
    	  var callhospitalizationGenerate = function(){
				...
				columns = new Array();
	     		...
	     		columns.push({ "mDataProp": "firstName","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%",
				     			"render": function (data, type, full, meta) {
				     				return '<a href="${context}/membership/'+full.id+'/complete">'+full.lastName+', '+full.firstName+'</a>';
							  }		
	     		});
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.hospital.name","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.admitDate","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.expDisDate","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.planDesc","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.authNum","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		columns.push({ "mDataProp": "mbrHospitalizationList.0.priorAdmits","bSearchable" : true, "bSortable": true,"sClass": "center","sWidth" : "10%"  });
	     		
	     		
	     		var myTable = $("#membershipHospitalizationTable");
	     		var thead = myTable.find("thead");  
	     		$("#membershipHospitalizationTable th").each(function() {
    	  ...
    	  function callDatableWithChangedDropDown(){
    		   ...
    		   if ( $.fn.DataTable.isDataTable('#membershipHospitalizationTable') ) {
  						$('#membershipHospitalizationTable').DataTable().destroy();
			   }
				$('#membershipHospitalizationTable tbody').empty();
    		   GetMembershipByInsPrvdrHedisRule(insSelectValue,prvdrSelectValue, columns);
    	}  
  	  ...
  	  var datatable2RestMembership = function(sSource, aoData, fnCallback) {
		 ...
		 $.ajax( {
     	  ...
     	  GetMembershipByInsPrvdrHedisRule = function (insId, prvdrId, aoColumns) {
      		
  	        var oTable = $('#membershipHospitalizationTable').removeAttr( "width" ).dataTable({  
  	        	"sDom": 'Bfrtip',
	        	 "buttons": [
	        	             {
	        	                 extend: 'excelHtml5',
  	         ...
  	         "bDestroy" : true,	
     	     "sAjaxSource" : getContextPath()+'/reports/membershipHospitalization/list',
 ...
 function myFunction(id,lastName,firstName) 
{
 	...
 	$('#mbrHospitalizationTable').dataTable({
		...
		//	$( "#modal-body" ).html('');
			...
			$(".modal-title").html(lastName+","+firstName+" - Hospitalization Followup");
	...
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" id="hospitalizationGenerate" class="btn btn-default"
