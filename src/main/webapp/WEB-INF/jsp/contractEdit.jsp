...
<div class="panel-group">
	<div class="panel panel-success">
		<div class="panel-heading">
			${contractType} Details <span class="clrRed">${Message}</span>
		...
		<div class="panel-body">
			...
			<springForm:form method="POST" id="contract${pmpmRequired}"
				commandName="contract" action="${id}/contract/save.do"
				class="form-horizontal" role="form" enctype="multipart/form-data">
				<div class="col-sm-12">
					<div class="col-sm-3">
						<div class="form-group required">
							<label class="control-label col-sm-5" for="contractNBR">Contract
								NBR</label>
							<div class="col-sm-7">
								...
								<springForm:input path="contractNBR" class="form-control"
									id="contractNBR" maxlength="20" placeholder="Contract NBR" />
								<springForm:errors path="contractNBR"
					...
					<div class="col-sm-3">
						<div class="form-group required">
							<label class="control-label col-sm-5" for="startDate">Start
								Date</label>
							<c:choose>
								<c:when test="${pmpmRequired && insuranceRequired}">
									<div class="col-sm-7">
										<fmt:formatDate value="${contract.startDate}" var="dateString"
											pattern="MM/dd/yyyy" />
										<springForm:input value="${dateString}" var="startDate"
											path="startDate" class="form-control datepicker1"
											id="startDate" placeholder="Start Date" />
										<span class="startDateText col-sm-12 insText"></span>
										<springForm:errors path="startDate"
											cssClass="error text-danger" />
									</div>
								</c:when>
								<c:otherwise>
									<div class="col-sm-7">
										<fmt:formatDate value="${contract.startDate}" var="dateString"
											pattern="MM/dd/yyyy" />
										<springForm:input value="${dateString}" var="startDate"
											path="startDate" class="form-control datepickerfrom"
											id="from" placeholder="Start Date" />
										<springForm:errors path="startDate"
											cssClass="error text-danger" />
									</div>
								</c:otherwise>
					...
					<div class="col-sm-3">
						<div class="form-group required">
							<label class="control-label col-sm-5" for="endDate">End
								Date</label>
							<c:choose>
								<c:when test="${pmpmRequired && insuranceRequired}">
									<div class="col-sm-7">
										<fmt:formatDate value="${contract.endDate}" var="dateString"
											pattern="MM/dd/yyyy" />
										<springForm:input value="${dateString}" path="endDate"
											class="form-control datepicker3" id="endDate"
								...
								<c:otherwise>
									<div class="col-sm-7">
										<fmt:formatDate value="${contract.endDate}" var="dateString"
											pattern="MM/dd/yyyy" />
										<springForm:input value="${dateString}" path="endDate"
											class="form-control datepickerto" id="to"
											placeholder="End Date" />
										<springForm:errors path="endDate" cssClass="error text-danger" />
									</div>
								</c:otherwise>
					...
					<div class="col-sm-3">
						<div class="form-group">
							<label class="control-label col-sm-5" for="filesUpload">Contract
								file</label>
							<div class="col-sm-7">
								...
								<c:choose>
									<c:when test="${not empty contract.filesUpload.id}">
										<a href="#" onclick="fileDownload(${contract.id})"><span
											class="glyphicon glyphicon-open-file"></span></a>
									</c:when>
								</c:choose>
						...
						<c:when test="${insuranceRequired}">
							<div class="col-sm-3">
								<div class="form-group required">
									...
									<div class="col-sm-7">
										<springForm:select path="insId" class="form-control"
											id="status" onchange="return insPmpm(this.value)">
											<springForm:option value="${null}" label="Select One" />
											<springForm:options items="${insuranceList}" itemValue="id"
												itemLabel="name" />
										</springForm:select>
										<springForm:errors path="insId"
											cssClass="error text-danger col-sm-12" />
									</div>
								</div>
							</div>
						</c:when>
							...
							<div class="col-sm-3">
								<div class="form-group required">
									<label class="control-label required col-sm-5"
										for="pcpPrvdrNBR">PCP Prvdr NBR</label>
									<div class="col-sm-7">
										<springForm:input path="pcpPrvdrNBR" class="form-control"
											id="pcpPrvdrNBR" placeholder="pcpPrvdrNBR" />
										<span class="pcpPrvdrNBR col-sm-12"></span>
										<springForm:errors path="pcpPrvdrNBR"
											cssClass="error text-danger" />
									</div>
								</div>
							</div>
