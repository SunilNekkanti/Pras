 ...
 */
@Repository
public class LeadMembershipClaimDaoImpl extends HibernateBaseDao<LeadMembershipClaim, Integer> implements LeadMembershipClaimDao {

	private static final Logger LOG = LoggerFactory.getLogger(LeadMembershipClaimDaoImpl.class);
	...
	/* (non-Javadoc)
	 * @see com.pfchoice.core.dao.LeadMembershipClaimDao#getPage(int, int, int)
	 */
	@Override
	public Pagination getPage(final  int pageNo, final  int pageSize, final int leadMbrId){
		if(leadMbrId >0){
			Criteria crit = createCriteria().createAlias("leadMbr", "leadMbr")
					.createAlias("leadMbrClaimDetailsList", "leadMbrClaimDetails")
					.createAlias("leadMbrClaimDetails.cpt", "cpt");
			crit.add(Restrictions.eq("activeInd", 'Y'));
			crit.add(Restrictions.eq("leadMbr.id", leadMbrId));
			return findByCriteria(crit, pageNo, pageSize);
		}else{
	...
	public LeadMembershipClaim deleteById(final Integer id) {
		LeadMembershipClaim entity = super.get(id);
		if (entity != null) {
	 ...
	 */
	@Override
	protected Class<LeadMembershipClaim> getEntityClass() {
		return LeadMembershipClaim.class;
	}
	...
	//	String tableNames = "membership_claims,membership_claim_details,unprocessed_amg_claims,unprocessed_amg_pharmacy_claims";
		String tableNames = "unprocessed_amg_claims,unprocessed_amg_pharmacy_claims";
		String[] tokens = tableNames.split(",", -1);
		for(String tableName :tokens){
			try {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<LeadMembershipClaim> getUnwantedClaims(final Integer insId, final Integer prvdrId, final Integer reportMonth, final Integer activityMonth,final Boolean isUnwanted){
